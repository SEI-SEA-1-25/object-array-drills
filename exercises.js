...
if(talkingHeadsAlbums.length >= 6){
  ...
  console.log("Talking heads didn't have muc houtput");
}
...
} else if (talkingHeadsAlbums.length%2 === 1){
...
function testArrayDivisibility(d1,d2,array){
  const Y = array.length;
  const d1_true = Y % d1 === 0;
  const d2_true = Y % d2 === 0;
  if(d1_true && d2_true){
    return(`The number ${Y} is divisble by ${d1} and ${d2}`);
  } else if (d1_true && !d2_true){
    return(`The number ${Y} is divisble by ${d1}`);
  } else if (!d1_true && d2_true){
    return(`The number ${Y} is divisble by ${d2}`);
  } else if (!d2_true && !d1_true){
    return(`The number ${Y} is not divisble by ${d1} or ${d2}`);
  } else{
    return("ERROR!")
  }
...
function arrayOfLength(i) { return Array.apply(null, Array(i)).map(function (x, i) { return i; })};

console.log(arrayOfLength(4));

console.log(testArrayDivisibility(2,3,arrayOfLength(0)));
console.log(testArrayDivisibility(2,3,arrayOfLength(1)));
console.log(testArrayDivisibility(2,3,arrayOfLength(2)));
console.log(testArrayDivisibility(2,3,arrayOfLength(6)));
console.log(testArrayDivisibility(2,3,arrayOfLength(7)));
console.log(testArrayDivisibility(2,3,arrayOfLength(9)));
...
for(i=0;i<talkingHeadsAlbums.length;i++){
  const albumDetails = talkingHeadsAlbums[i].albumDetails;
  album_label_is_sire = albumDetails.label === "Sire";
  album_labels_includes_sire = albumDetails.labels.includes("Sire")

  if(album_label_is_sire || album_labels_includes_sire){
