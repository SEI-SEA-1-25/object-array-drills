...
//    variable.
const recordLabel = album1.albumDetails.label
...
const vinyl = album2.albumDetails.formats[0];
album3.albumDetails.formats.push(vinyl);
...
var otherLabel = album6.albumDetails.labels[1]
album6.albumDetails.labels[1] = otherLabel.toUpperCase();
console.log(album6);
...
const labelsArr = album7.albumDetails.labels;
album7.albumDetails.labels = labelsArr.split();
console.log(album7);
...
for (dignitary of tickets) {
  let welcomeText = `Welcome, ${dignitary.name}!`;
  
  if(dignitary.seats === 1) {
      welcomeText += " You may sit anywhere"
  } else {
      welcomeText += " You and your party may sit anywhere"
  }

  if(dignitary.type === "premium") {
    welcomeText += ` in the first 3 rows of the ${dignitary.section} section.`
  } else {
    welcomeText += ` except the first 3 rows of the ${dignitary.section} section.`
  }
...
for (ticket of ticketsTwo) {
  ...
  if(drinkVoucher > 0) {
    ticketData += ` $${drinkVoucher} DRINKS`
  }
